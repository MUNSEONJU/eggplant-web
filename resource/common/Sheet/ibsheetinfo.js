...
function createIBSheet(sheetid, width, height, locale) 
{
  ...
  /*   hunel custom @     */
  div_str += "<div id='DIV_" + sheetid + "' style='width:100%;height:100%'>";
...
/* @ hunel custom [add] */
  if(typeof HCG != "undefined")
  {
    $("#__viewState").val(HCG.toJsonString(HCG.toQueryParams(plain_text)));
  }
  else
  {
...
function IBS_InitSheet(sheet, info) 
{
  ...
  if (!cInfo.Headers) 
  {
    cInfo.Headers = [{Text: "",Align: "Center"}];
  }
...
function IBS_CopyJson2Form(jsonobj, frmName) 
{
  ...
  if (typeof frmName == "object")  frmName = frmName.name;
