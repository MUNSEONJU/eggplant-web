...
<%
  ...
  // 예 : "C:/tmp/"   "/usr/tmp/"
  //====================================================================================================
  ibExcel.setTempFolder(application.getRealPath(""));
  ...
  try {
    ...
    if (bToken) {
      ...
      out.println("<script>try{var targetWnd = null; if(opener!=null) {targetWnd = opener;} else {targetWnd = parent;} targetWnd.Grids[targetWnd.Grids.g_targetExcelSheetID].ShowAlert('엑셀 다운로드중 에러가 발생하였습니다.[Server Busy]', 'U');}catch(e){}</script>");
    }
  } catch(IOException e) {
  ...
  } catch (Error e) {  // TODO Exception 처리 확인
    out.println("<script>try{var targetWnd = null; if(opener!=null) {targetWnd = opener;} else {targetWnd = parent;} targetWnd.Grids[targetWnd.Grids.g_targetExcelSheetID].ShowAlert('엑셀 파일을 읽는 도중 예외가 발생하였습니다.', 'U');}catch(e){}</script>");
